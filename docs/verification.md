...
## Publication-phase diagnostics

The [next repair attempt](https://github.com/szymboot/discord-bot/actions/runs/34294190185) no longer reported an evidence validation category, but failed somewhere in validate-and-publish. The live finding remained unresolved and no new review appeared. The previous error handler discarded the exception category, so this run does not identify a root cause.

Read-only inspection confirmed the thread is still App-owned and that the exact node query from the action is syntactically accepted by GitHub. Publication now reports fixed labels for snapshot refresh, history comparison, resolving a finding, off-diff creation, status publication and review submission. API failures retain HTTP status and allowlisted GraphQL error types; no response messages, paths, tokens or raw model output are logged. Thread resolution additionally requires the returned thread ID and isResolved=true before continuing toward approval.

47 local tests pass, including HTTP/GraphQL/network diagnostics, secret redaction, operation attribution, and affirmative/negative resolution responses. These changes repair the diagnostic gap and enforce mutation confirmation; they do not establish the cause of the previous live publication failure. A new pinned run is necessary. No discord-bot changes or live thread mutations were performed by the fork agent.

## Resolve authorization requirement

The [diagnostic repair run](https://github.com/szymboot/discord-bot/actions/runs/34294894916) identified `publication=graphql; operation=resolve-finding; graphql_types=FORBIDDEN`. The installation has Contents write and Pull requests write, but the generated token was narrowed to Contents read. An [identical reported reproduction](https://github.com/github/gh-aw/issues/35726) shows that resolveReviewThread requires Contents write for App installation tokens, in addition to Pull requests write.

The concrete configuration remedy is `permission-contents: write` in the reviewer token-generation step, keeping target-repository scoping and `permission-pull-requests: write`. Do not expand GITHUB_TOKEN permissions, change the App installation, rotate secrets, use a PAT, manually resolve the finding, or weaken evidence/ownership/SHA checks. The fork's generator and configuration documentation have been corrected; the bot agent was asked to run the repaired source with that token permission. Runtime action SHA 0ee026027e988bb9093a1fc10b5dfae5f9067f07 can test this remedy without a runtime code change. Live confirmation is pending.
